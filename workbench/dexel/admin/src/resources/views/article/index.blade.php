...
<nav class="navbar dexel-coach-navbar">
            <div class="container-fluid">
               <div class="navbar-header">
                  <a class="navbar-brand header-brand" href="#">Article Uploads</a>
               </div>
            </div>
         </nav>
...
<button type="button" class="btn btn-info btn-lg" data-toggle="modal" id="add_article">Add article</button>
                        ...
                        <th class="va-m">Created date</th>
                         ...
                         <th class="va-m">Action</th>
                                        ...
                                        <tr>
                                          ...
                                          <td>{{$article->essence->title}}</td>
                                          <td>{{$article->created_at}}</td>
                                          <td>
                                            <a href='javascript:void(0)' class='fa fa-pencil' title='Edit' data-toggle='modal' data-target='#myModal'></a>
                                            <a href='javascript:void(0)' class='fa fa-eye view_article' title='View' data-slug="{{$article->slug}}" ></a>
                                            <a href='javascript:void(0)' data-slug="{{$article->slug}}" title='Delete' class='fa fa-trash-o remove_article'></a>
                                          </td>
                                        </tr>
                                        @endforeach
                                        </tbody>
...
<div id="article_details" class="modal fade bg" role="dialog">
...
@include("texteditor.js")
 ...
 $(document).ready(function()
 {
  ...
  $.ajax({
    ...
    $('#page').on('click', '.view_article', function(event)
    {
      event.preventDefault();
      var url = "{{url('coach/view/article')}}";
      $.get( url+"/"+$(this).attr('data-slug'), function( data ) {
        $("#article_details").html(data);
        $("#article_details").modal('show');
      });
    });
    $('#page').on('click', '.remove_article', function(event)
    {
      event.preventDefault();
      var r = confirm("Are you sure about remove this item?");
      if (r == true)
      {
        var url = "{{url('coach/remove/article')}}";
        $.get( url+"/"+$(this).attr('data-slug'), function( data ) {
          location.reload();
        });
      }
